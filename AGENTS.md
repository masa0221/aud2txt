...
Shell script that transcribes audio files (MP3, WAV, M4A, etc.) to text. Uses OpenAI Whisper when available; otherwise outputs placeholder .txt files.
...
7. CI runs on PR (Docker build + transcribe test)
...
2. `brew tap masa0221/tap` → `brew bump-formula-pr --no-fork masa0221/tap aud2txt`
